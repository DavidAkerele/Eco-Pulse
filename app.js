...
function renderBudgetUI() {
  ...
  if (alertBox && alertIcon && alertTitle && alertDesc) {
    ...
    if (isOver) {
      ...
      alertBox.classList.add('budget-alert-safe');
      alertIcon.setAttribute('data-lucide', 'shield-check');
      alertIcon.className = 'budget-alert-icon text-green';
      alertTitle.textContent = 'Budget Safe';
      alertDesc.textContent = `You're well within your daily CO₂ limit — ${remaining.toFixed(3)}g remaining. Keep using eco-friendly models!`;
    }
    lucide.createIcons();
  }
